...
1. Зарегистрируйтесь в личном кабинете [Менеджер Платформы](https://platform.2gis.ru/ru/?utm_source=docs&utm_medium=referral&utm_campaign=api_dm).
...
Если хотя бы одно из этих ограничений не выполнено, запрос вернёт ошибку. Для выполнения запросов с такими входными данными используйте [асинхронный метод](#large-numbers-of-points).
...
Подробную информацию о полях ответа можно посмотреть в [Справочнике API](/api/navigation/distance-matrix/reference).
...
#### Пример запроса {#asynchronous-request-example}
...
Запрос вернёт текущий статус задачи и ссылку на файл с решением, если задача была успешно завершена. Структура файла с решением совпадает со структурой ответа при использовании [синхронного запроса](#response-example).
