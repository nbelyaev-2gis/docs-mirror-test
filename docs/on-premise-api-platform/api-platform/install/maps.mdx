...
2. Убедитесь, что выполнены необходимые предварительные шаги:
   ...
   5. [Установка прокси для API пробок](/on-premise-api-platform/api-platform/install/trafficproxy)
...
4. Убедитесь, что удовлетворены требования к ресурсам, приведённые в Helm-чартах:
   ...
   * [Styles API](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/styles-api/values.yaml).
   ...
   Подробнее о том, как это сделать, см. в документе [Системные требования](/on-premise-api-platform/api-platform/install/requirements).
...
2. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/tiles-api/README.md).
   ...
   * `importer`: настройки воркеров процесса импорта (Kubernetes Importer job).
     ...
     * `workerResources`: настройки вычислительных ресурсов для воркера. Чтобы узнать рекомендуемые значения ресурсов, см. [Вычислительные ресурсы](/on-premise-api-platform/api-platform/install/requirements#resources).
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/styles-api/README.md).
   ...
   * `api.resources`: настройки вычислительных ресурсов для сервиса. Чтобы узнать рекомендуемые значения ресурсов, см. [Вычислительные ресурсы](/on-premise-api-platform/api-platform/install/requirements#resources).
...
Чтобы подключить пользовательский стиль карты, установите [Менеджер Платформы](/on-premise-api-platform/api-platform/install/platform) и выполните шаги по [загрузке и применению стиля](/on-premise-api-platform/api-platform/user-guide/platform-manager#add-map-style).
