...
const defaultCoverPageFunction: PageFunction = (siteConfig, _pluginConfig, pageTitle, _version) => {
  // Как-то вытащить текущую локаль из siteConfig?
    ...
    <head>
      ...
      <body>
        <div style="margin: 2cm;">
          <h1 style="color:#005479;font-size:28px;font-family:sans-serif;font-weight:bold">${
            siteConfig.projectName
          }<h1>
          <h2 style="color:#005479;font-size:16px;font-family:sans-serif;">${
          ...
          <dl style="font-family:sans-serif;margin-top:10em;display: flex; flex-flow: row; flex-wrap: wrap; width: 600px; overflow: visible;color:#005479;font-size:12px;font-weight:normal;">
            ...
            <dd style="margin-top:1em;flex:0 0 80%; margin-left: auto; text-align: left;text-overflow: ellipsis; overflow: hidden;">${
              _pluginConfig.author
            }</dd>
            <dt style="margin-top:1em;flex: 0 0 20%; text-overflow: ellipsis; overflow: hidden;">Date:</dt>
            <dd style="margin-top:1em;flex:0 0 80%; margin-left: auto; text-align: left;text-overflow: ellipsis; overflow: hidden;">${date}</dd>
          </dl>
        </div>
      </body>
...
const defaultPageHeaderFunction: PageFunction = (
  _siteConfig,
  pluginConfig,
  pageTitle,
  _version,
...
const defaultPageFooterFunction: PageFunction = (
  _siteConfig,
  pluginConfig,
  _pageTitle,
  _version,
  ...
  return `
    <div style="height:1cm;display:flex;margin: 0 1.5cm;color: #005479;font-size:9px;font-family:sans-serif;width:100%;">
      ...
      <span style="flex-grow: 1; width: 33%; text-align:center;">${new Date()
        .toISOString()
        .substring(0, 10)}</span>
...
export function processOptions(options: PluginOptions | undefined): PapersaurusPluginOptions {
  const pluginOptions = { ...options } as PapersaurusPluginOptions;
  if (!pluginOptions.footerParser) {
    pluginOptions.footerParser = RegExp(
      `© ${pluginOptions.author}\\d{4}-\\d{2}-\\d{2}Page \\d* \\/ \\d*`,
      'g',
    );
  }
