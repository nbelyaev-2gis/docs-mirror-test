...
/**
 * Источник: форк репы docusaurus-plugin-papersaurus: https://github.com/Alberto-Carrillo-flexxible/docusaurus-plugin-papersaurus/tree/v3.x-migration
 * Оригинальная репа тут: https://github.com/simologos/docusaurus-plugin-papersaurus
 */

import { LoadContext, Plugin } from '@docusaurus/types';
import { generatePdfFiles } from './generate';
import { PluginOptions } from './types';
import { processOptions, validateOptions } from './validateOptions';

export const pdfGeneratorPlugin = (
  ...
  const validatedOptions = validateOptions({
    ...
    validate: (schema, options) => {
      ...
      throw new Error(`Options validation failed: ${result.error?.message}`);
    },
  });
  const pluginOptions = processOptions(validatedOptions);
  ...
  return {
          ...
          const checkAndInsertPdfButtons = function () {
            ...
            if (!$("#pdfLink").length) {
              var pdfDownloadButton = $('' +
                ...
                '  <a class="navbar__item navbar__link pdfLink" id="pdfLink" href="#">${
                  pluginOptions.downloadButtonText
                }</a>' +
            ...
            if (!$("#pdfLinkSidebar").length) {
              var pdfDownoadButtonSidebar = $('<li class="menu__list-item menu__list-item--collapsed" id="pdfLinkSidebar"><a role="button" class="menu__link menu__link--sublist">${
                pluginOptions.downloadButtonText
              }</a><ul class="menu__list" id="pdfLinkSidebarMenu" style=""></ul></li>');
    ...
    async postBuild(props) {
      const forceBuild = process.env.BUILD_PDF || '';
      if (
