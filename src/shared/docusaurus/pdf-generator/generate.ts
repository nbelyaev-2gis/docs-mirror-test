...
import { PapersaurusPluginOptions, TocInfo } from './types';
...
function createBrowser() {
  console.log(`${pluginLogPrefix}Creating browser...`);
...
export async function generatePdfFiles(
  ...
  if (docsPlugins.length > 1 || docsPlugins.length == 0) {
    throw new Error(
      `${pluginLogPrefix}Too many or too few docs plugins found, only 1 is supported.`,
    );
  }
  const docPlugin: LoadedPlugin = docsPlugins[0];
  ...
  const createNewBrowser = async () => {
    console.log(`${pluginLogPrefix}Restarting browser...`);
    // Убиваем процесс браузера, вместо вызова browser.close(), т.к. он намертво зависает.
    // Проблема известная: https://github.com/Sparticuz/chromium/issues/85
  ...
  const linkToFile: {
    [key: string]: { label: string; file: string; type: 'root' | 'section' | 'chapter' }[];
  } = {};

  // Loop through all found versions
  for (const versionInfo of (docPlugin.content as LoadedContent).loadedVersions) {
    if (
      pluginOptions.versions.length != 0 &&
      !pluginOptions.versions.includes(versionInfo.versionName)
    ) {
    ...
    for (const [i, sidebarName] of pluginOptions.sidebarNames.entries()) {
      ...
      const versionPath = getVersionPath(versionInfo, siteConfig);
      const versionPdfPath = [pdfPath, versionPath, folderName]
        .filter((str) => str != '')
        .join('/');
      ...
      const sidebar = versionInfo.sidebars[sidebarName];
      if (sidebar) {
        ...
        console.log(
          `${pluginLogPrefix}Sidebar '${sidebarName}' doesn't exist in version '${versionInfo.label}', continue without it...`,
        );
      }
    }
  }
...
function saveUrlToFileMappingsRecursive(
  ...
  for (const item of sideBarItems) {
    if (item.permalink) {
      output[stripTrailingSlash(item.permalink)] = [
        ...parents,
        {
          label: item.label,
    ...
    if (item.items) {
      saveUrlToFileMappingsRecursive(item.items, output, [
        ...parents,
        {
          label: item.label,
          file: item.pdfFilename,
...
async function createPdfFilesRecursive(
  ...
  createNewBrowser: () => Promise<puppeteer.Browser>,
  siteAddress: string,
  productTitle: string,
  ...
  let pdfFilename = pluginOptions.getPdfFileName(
    siteConfig,
    pluginOptions,
    sideBarItem.label,
    sideBarItem.unversionedId,
    parentTitles,
    parentIds,
    version.versionName,
    version.path,
  ...
  if (articles.length > 0) {
    ...
    // В CI есть неприятный баг: браузерный процесс зависает и все последующие ручки возвращают
    // ошибку "ProtocolError: Target.createTarget timed out. Increase the 'protocolTimeout' setting".
    // В качестве фикса пересоздаём браузерный инстанс, если встречаемся с ошибкой.
    // Решение взял отсюда: https://github.com/puppeteer/puppeteer/issues/10144#issuecomment-1918840262
    ...
    do {
      ...
      try {
        ...
        console.error(
          `${pluginLogPrefix}Error creating PDF ${buildDir}/${pdfFilename}.pdf: ${error}`,
        );

        if (retryCount > 0) {
          console.log(`${pluginLogPrefix}Retrying...`);
...
function readHtmlForItem(
  ...
  const htmlFileContent: string = fs.readFileSync(htmlFilePath, { encoding: 'utf8' });
...
async function createPdfFromArticles(
  ...
  const stylePath = articleList[0].stylePath;
  const scriptPath = articleList[0].scriptPath;
...
const decodeHtml = (str: string) => {
  ...
  return (
    str
      ...
      .replace(/\&([^;]+);/g, function (entity, entityCode) {
        let match;
...
const getPageWithFixedToc = (
  ...
  const linesTOC = linesArray.slice(1, 1 + tocList.length);
...
function hasOwnProperty<X extends Record<PropertyKey, unknown>, Y extends PropertyKey>(
  obj: X,
  prop: Y,
